...
class Details(Resource):
    def get(self, platform,username):
        ...
        if platform == 'run':
            ...
            # #records = db.catches
            # try:
            #     file = fs.find_one({'filename': 'king6.jpg'})
            #     image = file.read()
            #     outfilename = "img.jpg" 
            #     output= open(outfilename,"wb") 
            #     output.write(image)
            ...
            mori = 0
            for x in img:
                os.system("python detect.py --weights ./runs/train/exp9/weights/best.pt --source {} --conf 0.1 --save-txt".format(x))
                exp = glob.glob('./runs/detect/*/')
                print(exp)
                exp.remove('./runs/detect/exp/')
                exp = sorted(exp,key = lambda x:int(x[17:-1:]),reverse=True)
            ...
            return {'status': 'Sucess', 'result': {"catla":catlap,"rohu":rohup,"mori":morip}}
        if platform == 'get':
